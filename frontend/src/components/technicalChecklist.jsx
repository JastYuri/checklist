...
const TechnicalChecklist = ({ data, onChange, onSave }) => {
  ...
  return (
    <div className="card bg-base-200 shadow-md p-4 md:p-6">
      ...
      <div className="mb-6">
        <h5 className="text-md font-semibold mb-3 text-sm md:text-base">I. Break Testing (Breaking Force daN)</h5>
        
        <div className="mb-4 overflow-x-auto">
          <table className="table w-full border min-w-150">
            <tbody>
              <tr><th colSpan="4" className="text-center border font-bold">Maximum Breaking Force</th></tr><tr><th className="border text-sm md:text-base">Front (Left Hand)</th><th className="border text-sm md:text-base">Front (Right Hand)</th><th className="border text-sm md:text-base">Sum</th><th className="border text-sm md:text-base">Front Difference</th></tr><tr><td className="border"><input type="number" value={technicalData.breakingForce?.max?.front?.left || ''} onChange={(e) => updateBreakingForce('max', 'front', 'left', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.max?.front?.right || ''} onChange={(e) => updateBreakingForce('max', 'front', 'right', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.max?.front?.sum || ''} onChange={(e) => updateBreakingForce('max', 'front', 'sum', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.max?.front?.difference || ''} onChange={(e) => updateBreakingForce('max', 'front', 'difference', e.target.value)} className="input input-bordered w-full" /></td></tr>
              <tr><th className="border text-sm md:text-base">Rear (Left Hand)</th><th className="border text-sm md:text-base">Rear (Right Hand)</th><th className="border text-sm md:text-base">Sum</th><th className="border text-sm md:text-base">Rear Difference</th></tr><tr><td className="border"><input type="number" value={technicalData.breakingForce?.max?.rear?.left || ''} onChange={(e) => updateBreakingForce('max', 'rear', 'left', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.max?.rear?.right || ''} onChange={(e) => updateBreakingForce('max', 'rear', 'right', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.max?.rear?.sum || ''} onChange={(e) => updateBreakingForce('max', 'rear', 'sum', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.max?.rear?.difference || ''} onChange={(e) => updateBreakingForce('max', 'rear', 'difference', e.target.value)} className="input input-bordered w-full" /></td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="table w-full border min-w-150">
            <tbody>
              <tr><th colSpan="4" className="text-center border font-bold">Minimum Breaking Force</th></tr><tr><th className="border text-sm md:text-base">Front (Left Hand)</th><th className="border text-sm md:text-base">Front (Right Hand)</th><th className="border text-sm md:text-base">Sum</th><th className="border text-sm md:text-base">Front Difference</th></tr><tr><td className="border"><input type="number" value={technicalData.breakingForce?.min?.front?.left || ''} onChange={(e) => updateBreakingForce('min', 'front', 'left', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.min?.front?.right || ''} onChange={(e) => updateBreakingForce('min', 'front', 'right', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.min?.front?.sum || ''} onChange={(e) => updateBreakingForce('min', 'front', 'sum', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.min?.front?.difference || ''} onChange={(e) => updateBreakingForce('min', 'front', 'difference', e.target.value)} className="input input-bordered w-full" /></td></tr>
              <tr><th className="border text-sm md:text-base">Rear (Left Hand)</th><th className="border text-sm md:text-base">Rear (Right Hand)</th><th className="border text-sm md:text-base">Sum</th><th className="border text-sm md:text-base">Rear Difference</th></tr><tr><td className="border"><input type="number" value={technicalData.breakingForce?.min?.rear?.left || ''} onChange={(e) => updateBreakingForce('min', 'rear', 'left', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.min?.rear?.right || ''} onChange={(e) => updateBreakingForce('min', 'rear', 'right', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.min?.rear?.sum || ''} onChange={(e) => updateBreakingForce('min', 'rear', 'sum', e.target.value)} className="input input-bordered w-full" /></td><td className="border"><input type="number" value={technicalData.breakingForce?.min?.rear?.difference || ''} onChange={(e) => updateBreakingForce('min', 'rear', 'difference', e.target.value)} className="input input-bordered w-full" /></td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="table w-full border min-w-100">
            <thead>
              <tr>
      ...
      <div className="mb-6">
            ...
            <tbody>
              {technicalData.slipTester?.map((slip, idx) => (
                <tr key={idx}>
                  ...
                  <td><button className="btn btn-sm btn-error w-full md:w-auto" onClick={() => deleteSlipTester(idx)}>Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex justify-end mt-2">
          <button className="btn btn-sm btn-outline w-full md:w-auto" onClick={addSlipTester}>Add Slip Test</button>
        </div>
      </div>
      ...
      <div className="mb-6">
            ...
            <tbody>
              {technicalData.absTesting?.map((abs, idx) => (
                <tr key={idx}>
                  ...
                  <td><button className="btn btn-sm btn-error w-full md:w-auto" onClick={() => deleteAbsTesting(idx)}>Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex justify-end mt-2">
          <button className="btn btn-sm btn-outline w-full md:w-auto" onClick={addAbsTesting}>Add ABS Test</button>
        </div>
      </div>

      <div className="flex flex-col md:flex-row justify-end mt-4 space-y-2 md:space-y-0 md:space-x-2">
        <button className="btn btn-success w-full md:w-auto" onClick={() => onSave(technicalData)}>Save Technical Checklist</button>
      </div>
    </div>
  );
};
